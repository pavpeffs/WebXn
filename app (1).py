...
def aggregate_bookings(df):
    ...
    expected_sub_count = {
        ...
        "Cameron Bank": 2, "East (winter)": 4, "South": 3, "Astro 1": 2, "Astro 2": 2, "Muga": 3,
        "Cricket Nets": 4
    }
    aggregated_rows = []
...
with tabs[0]:
    ...
    if df is None:
        ...
        date_options = sorted(df.iloc[:,0].str.split(' - ', expand=True)[0].unique())
        # (For simplicity, we extract the date from the first column of df_extract.)
        # Here we reconstruct df_processed based on known columns (assuming columns 24 to 30 were extracted earlier)
        ...
        filtered_df = df_processed.copy()  # In a real scenario, add your filter logic here.
...
with tabs[1]:
    ...
    if df is None:
        ...
        # Use the same processed data (df_processed) from the Daily Overview tab
